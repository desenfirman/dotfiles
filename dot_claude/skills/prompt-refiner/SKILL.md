---
name: prompt-refiner
description: Prompt Engineering & Prompt Refiner skill. Transforms raw or messy user requests into concise, token-efficient, high-performance master prompts for systems like GPT, Claude, and Gemini. Use this skill when the user wants to design, improve, compress, refactor, or optimize a prompt — even if they phrase it casually like "make this prompt better", "write me a good prompt for X", "turn this into a system prompt", or "this prompt isn't working well". Also trigger when the user provides a long, noisy, or unclear prompt and wants it cleaned up for a specific AI system. When in doubt, assume they want a better prompt and proceed.
---
...
- Wants to **design, improve, compress, or refactor a prompt**, for example:
  - "Help me write a better prompt for GPT/Claude/Gemini."
  - "Optimize this prompt to be more accurate and token-efficient."
  - "Create a solid prompt for doing X (coding, writing, analysis, etc.)."
- Provides:
...
2. **⚡ Optimized Request**
   ...
   - Output this block inside a fenced code block using triple backticks:
     ```text
   ...
   - Inside this block:
     ...
     - Do NOT add explanation or commentary before, inside, or after the code block.
...
- Mirror the **user's language** for explanations and improvement questions.
...
1. **Goal Alignment** — Does the optimized prompt clearly target the user's core problem?
2. **Token Efficiency** — Did you remove redundancy and filler? Are all longer sections necessary?
3. **Structure & Completeness** — Are all PCTCE+O elements present? Is the output format correct?
4. **Hallucination Controls** — Does the prompt instruct the target AI on handling uncertainty?
